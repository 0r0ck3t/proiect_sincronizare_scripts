...
def transition_issue(jira_url, issue_key, auth, target_name):
    ...
    if resp.status_code != 200:
        raise RuntimeError(f"GET transitions failed {resp.status_code}: {resp.text}")

    try:
        payload = resp.json()
    except ValueError:
        raise RuntimeError(f"GET transitions did not return JSON: {resp.text!r}")

    # Jira normal: {"transitions": [...]}; fallback: [...] (în cazuri atipice/proxy)
    if isinstance(payload, dict) and "transitions" in payload:
        transitions = payload["transitions"]
    elif isinstance(payload, list):
        transitions = payload
    else:
        raise RuntimeError(f"No 'transitions' key and not a list: {payload}")

    # (optional) debug
    # print(transitions)

    # 2) Caută tranziția fie după numele tranziției, fie după statusul 'to'
    target_lower = target_name.lower()
    t = next((
        t for t in transitions
        if (t.get("to", {}).get("name", "").lower() == target_lower)   # status țintă
        or (t.get("name", "").lower() == target_lower)                  # numele tranziției
    ), None)

    if not t:
        available_to = [t.get("to", {}).get("name") for t in transitions]
        available_tr = [t.get("name") for t in transitions]
        raise ValueError(
            f"Nu am găsit o tranziție către/denumită '{target_name}'. "
            f"Statusuri 'to' disponibile: {available_to}; Tranziții disponibile: {available_tr}"
        )
...
# for row in db.get_status_sync_to_jira(cnt6):
